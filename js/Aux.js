...
function exportData(){// downloads a json file with the data

      var json = JSON.stringify(training_data);
      var blob = new Blob([json], {type: "application/json"});

        if (navigator.msSaveBlob) { // IE 10+
          navigator.msSaveBlob(blob, 'training_data.json');
        } else {
          var a = document.createElement("a");
                if (a.download !== undefined) { // feature detection
                    ...
                    a.setAttribute("download", 'training_data.json');
